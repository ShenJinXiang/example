	...
	$.extend($.validator, {
		...
		prototype : {
			...
			elements : function() {
				...
				// :reset, :image,
				// [disabled]").not(this.settings.ignore).filter(function(){!this.name&&validator.settings.debug&&window.console&&console.error("%o
				// has no name assigned",this);if(this.name in
				// rulesCache||!validator.objectLength($(this).rules()))return
				// false;rulesCache[this.name]=true;return true;});
						...
						.not(this.settings.ignore).filter(function() {
							!this.name
									...
									&& console.error("%o has no name assigned",
											this);
							/**
							 * 此处注释为了解决 table中多行input name相同时只校验第一个的问题
							 */
